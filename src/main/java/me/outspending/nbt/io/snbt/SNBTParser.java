...
import me.outspending.nbt.io.StringPointer;
import me.outspending.nbt.io.exceptions.ParseException;
...
public class SNBTParser implements MaxDepthIO {
    ...
    public NBTTag<?> parse(int maxDepth, boolean lenient) throws ParseException {
        NBTTag<?> tag = parseAnything(maxDepth);
        if (!lenient) {
            ptr.skipWhitespace();
            if (ptr.hasNext()) {
    ...
    public NBTTag<?> parse(int maxDepth) throws ParseException {
        return parse(maxDepth, false);
    }

    public NBTTag<?> parse() throws ParseException {
        return parse(NBTTag.DEFAULT_MAX_DEPTH, false);
    }
    ...
    private NBTTag<?> parseAnything(int maxDepth) throws ParseException {
        ptr.skipWhitespace();
    ...
    private NBTTag<?> parseStringOrLiteral() throws ParseException {
        ptr.skipWhitespace();
        if (ptr.currentChar() == '"') {
            return new NBTString(ptr.parseQuotedString());
        }
    ...
    private NBTCompound parseCompoundTag(int maxDepth) throws ParseException {
        ptr.expectChar('{');
    ...
    private NBTList<?> parseListTag(int maxDepth) throws ParseException {
        ptr.expectChar('[');
        ptr.skipWhitespace();
        NBTList<?> list = NBTList.createUnchecked(NBTEnd.class);
        while (ptr.currentChar() != ']') {
            NBTList<?> element = (NBTList<?>) parseAnything(decrementMaxDepth(maxDepth));
            try {
    ...
    private NBTByteArray parseByteArrayTag() throws ParseException {
    ...
    private NBTIntArray parseIntArrayTag() throws ParseException {
        ...
        ptr.expectChar(']');
        return new NBTIntArray(intList.stream().mapToInt(i -> i).toArray());
    }

    private NBTLongArray parseLongArrayTag() throws ParseException {
        ...
        ptr.expectChar(']');
        return new NBTLongArray(longList.stream().mapToLong(l -> l).toArray());
    }
}
